...
public class Chessboard extends JPanel
{
    ...
    public int get_width(boolean includeLables)
    {
    ...
    {
        super.paintComponent(g); // Call the superclass's paintComponent method to ensure proper painting behavior
    ...
    protected final void drawLabels(int square_height)
    {
        ...
        if (this.settings.upsideDown)
        {
            for (int i = 1; i <= 8; i++)
            {
                uDL2D.drawString(Integer.valueOf(i).toString(), 3 + (labelHeight / 3), (square_height * (i - 1)) + addX);
            }
        }
        else
        {
            ...
            for (int i = 8; i > 0; i--, j++)
            {
                uDL2D.drawString(Integer.valueOf(i).toString(), 3 + (labelHeight / 3), (square_height * (j - 1)) + addX);        }
        uDL2D.dispose();
